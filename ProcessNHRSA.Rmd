...
#  validmeta=c(7,6,3,8,9)
  ...
  repealed=FALSE
  if (lmeta %in% c(8,9)){
    if (lmeta==8) { names(rsameta)=dftags8
    }
    else {
      names(rsameta)=dftags9
    }
  ...
  if (sum(rsameta %like% "repealed")==0){
    df=data.frame(rsaxml=as.character(rsahtml[rsarec]),
                  titlenum=rsameta$title.h11,
                  titlename=rsameta$title.h13,
                  chapternum=rsameta$chapter.h21,
                  chaptertitle=rsameta$chapter.h23,
